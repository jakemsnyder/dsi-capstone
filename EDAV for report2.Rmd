...
---
I'd recommend showing how the values in our dataset compare to the population data. The fact that our dataset is 80% white isn't interesting if the population is also 80% white (it's not, which is why it's interesting). Or perhaps show the difference in percent (i.e. white is +12%, black is -8%, hispanic -12%... these are made up numbers but you get what I mean)
...
ggplot(data=plot_demographics, aes(x=as.factor(paste(Cat, Label, sep='\n')),
                                   y=Prop,colour = as.factor(Type), group=as.factor(Type), linetype=as.factor(Type))) +
  ...
  scale_fill_manual(values=c('#1f78b4','#33a02c', '#e31a1c', '#ff7f00', '#8856a7'),guide=FALSE) +
    scale_color_manual(name = 'Guide1', values=c('#FF8B8B','#000000','#C6d7d7')) +
