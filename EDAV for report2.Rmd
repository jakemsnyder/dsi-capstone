...
ggplot(data=plot_demographics, aes(x=as.factor(paste(Cat, Label, sep='\n')),
  ...
  scale_fill_manual(values=c('#1f78b4','#33a02c', '#e31a1c', '#ff7f00', '#8856a7'),guide=FALSE) +
    scale_color_manual(name = 'Guide1', values=c('#FF8B8B','#000000','#008000')) +
...
generic_map_df <- cbind(generic_map_df, dem, other)%>%
    select(state, total_num_responses, num_rep, num_dem, num_other) %>%
    group_by(state) %>%
    mutate(prop_rep = num_rep / total_num_responses) %>% 
    mutate(prop_dem = num_dem / total_num_responses) %>% 
    mutate(prop_other = num_other / total_num_responses)
...
specific_map_df <- cbind(specific_map_df, dem, other)%>%
    select(state, total_num_responses, num_rep, num_dem, num_other) %>%
    group_by(state) %>%
    mutate(prop_rep = num_rep / total_num_responses) %>% 
    mutate(prop_dem = num_dem / total_num_responses) %>% 
...
p <- plot_geo(generic_map_df, locationmode = 'USA-states') %>%
  ...
  layout(
    title = '% of Generic Survey Respondents likely to vote Republican',
    geo = geo
  )
...
p2 <- plot_geo(specific_map_df, locationmode = 'USA-states') %>%
  ...
  colorbar(title = "% of Respondents likely to vote Republican", limits=c(0,1)) %>%
  layout(
    title = '% of Specific Survey Respondents likely to vote Republican',
    geo = geo
  )
p2
```
